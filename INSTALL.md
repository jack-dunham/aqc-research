...
- Using `pyenv-virtualenv` after installing [pyenv](https://github.com/pyenv/pyenv):

    ```sh
    pyenv install <PYTHON-VERSION>
    pyenv virtual-env <PYTHON-VERSION> <VENV-NAME>
...
```sh
pip install aqc-research-qduc@git+https://github.com/bjader/aqc-research-qduc
```

Updates can be retrieved at any point via:

```sh
pip install -U aqc-research-qduc@git+https://github.com/bjader/aqc-research-qduc
```
...
1. From the terminal, clone the repository. This will add the provided URL to the list of remotes under the name `origin`:
    ...
    ```sh
    git clone git@github.com:bjader/aqc-research-qduc.git
    ```
    ...
    git remote add upstream git@github.com:bjader/aqc-research-qduc.git
    ```

2. Change directory to the freshly cloned repository:

    ```sh
    cd aqc-research-qduc
    ```
...
```sh
pip install [-U] aqc-research-qduc[<OPT-BUN>]
```

Or, for multiple optional bundles:

```sh
pip install [-U] aqc-research-qduc[<OPT-BUN-1>,<OPT-BUN-2>]
```

The same format can be used for installation of these bundles from _local_ source by simply substituting `aqc-research-qduc` for `.` in the install commands:
...
```sh
pip install [-U] aqc-research-qduc@git+git@github.com:bjader/aqc-research-qduc.git[<OPT-BUN-1>,<OPT-BUN-2>]
```
...
>
> ```zsh
> pip install [-U] [-e] "aqc-research-qduc[<OPT-BUN>]"
> ```
>
...
>>> import aqc_research
>>> print(aqc_research)
<module 'aqc_research' from '.venv/lib/python3.10/site-packages/aqc_research/__init__.py'>
```
